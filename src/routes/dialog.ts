...
async function ensureFiles() {
  ...
  try { await fs.access(paths.answersJson); }
    ...
    await fs.writeFile(paths.answersJson, JSON.stringify({ form: FORM_ID, values: {} }, null, 2));
  }
}
...
async function computePlan() {
  ...
  if (res.code !== 0) throw new Error(res.stderr || res.stdout);
  const plan = JSON.parse(await fs.readFile(paths.plan, "utf8"));
  return plan;
}
...
/* -------------------- SHACL/shape lookups (cached) -------------------- */

type ShapeMeta = {
  datatype?: string;          // full IRI: xsd:integer | xsd:string | ...
  minInclusive?: number;
  maxInclusive?: number;
};

let SHAPES_CACHE: Map<string, ShapeMeta> | null = null;

async function loadShapesMeta(): Promise<Map<string, ShapeMeta>> {
  if (SHAPES_CACHE) return SHAPES_CACHE;

  const text = await fs.readFile(paths.shapes, "utf8");
  ...
  for (const q of quads) {
    ...
    if (!pathObj.value.startsWith(formNs)) continue;
    ...
    if (minQ?.object?.termType === "Literal") meta.minInclusive = Number(minQ.object.value);
    if (maxQ?.object?.termType === "Literal") meta.maxInclusive = Number(maxQ.object.value);
...
async function getFieldMeta(fieldId: string): Promise<ShapeMeta> {
...
function shortDatatype(iri?: string): "integer" | "boolean" | "string" | "decimal" | undefined {
  if (!iri) return undefined;
  if (iri === `${XSD}integer`) return "integer";
  if (iri === `${XSD}boolean`) return "boolean";
  if (iri === `${XSD}decimal` || iri === `${XSD}double` || iri === `${XSD}float`) return "decimal";
  if (iri === `${XSD}string`) return "string";
  return undefined;
}

function normalizeValue(raw: any, slot: NextSlot, meta: ShapeMeta) {
  ...
  switch (dt) {
    case "integer": {
      ...
      if (!Number.isFinite(n)) throw new Error(`Expected an integer for ${slot.id}`);
      v = n;
      break;
    }
    case "decimal": {
      const n = typeof v === "number" ? v : parseFloat(String(v));
      if (!Number.isFinite(n)) throw new Error(`Expected a number for ${slot.id}`);
      v = n;
      break;
    }
    case "boolean": {
      if (typeof v === "string") {
        ...
        if (["true", "yes", "1"].includes(low)) v = true;
        else if (["false", "no", "0"].includes(low)) v = false;
      }
      v = Boolean(v);
      break;
    }
...
export async function dialogStateHandler(_req: Request, res: Response) {
  try {
    await ensureFiles();
    await buildAnswers();
...
export async function dialogResetHandler(req: Request, res: Response) {
  try {
    await ensureFiles();
    const body = req.body || {};
    let answers = { form: FORM_ID, values: {} as Record<string, any> };
    try {
      answers = JSON.parse(await fs.readFile(paths.answersJson, "utf8"));
    } catch {}

    if (body.all) {
      answers.values = {};
    } else if (Array.isArray(body.clear)) {
      for (const k of body.clear) delete (answers.values as any)[k];
    } else if (typeof body.field === "string") {
      delete (answers.values as any)[body.field];
    }

    await fs.writeFile(paths.answersJson, JSON.stringify(answers, null, 2));
    await buildAnswers();
    const plan = await computePlan();
...
export async function dialogStepHandler(req: Request, res: Response) {
  try {
    ...
    if (req.body?.value !== undefined) {
      ...
      const next: NextSlot | undefined = nxtJson?.next;
      const id = next?.id;
