// src/dialog/normalizer.ts
type Option = { label: string; value: string | number };
type NextSlot = {
  id: string;                 // e.g. vehicleValue
  datatype?: string;          // e.g. "integer", "string", "boolean"
  min?: number;
  max?: number;
  options?: Option[];         // enumerations
};

export function normalizeValue(raw: any, slot: NextSlot): any {
  ...
  switch (slot.datatype) {
    case "integer":
      v = parseInt(v, 10);
      if (isNaN(v)) throw new Error(`Expected integer for ${slot.id}`);
      break;
    case "decimal":
      v = parseFloat(v);
      if (isNaN(v)) throw new Error(`Expected decimal for ${slot.id}`);
      break;
    case "boolean":
      if (typeof v === "string") {
